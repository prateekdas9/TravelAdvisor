...
class DayPlannerService{
  ...
  Future<DayPlannerModel> getDayPlannerDetails(String city) async {
    ...
    var url =
        "https://www.triposo.com/api/20200405/day_planner.json?location_id="+'$city';
    print('$url');
    var file = await MyCacheManager().getSingleFile(url, headers: requestHeaders);
    if (file != null && await file.exists()) {
      var res = await file.readAsString();
      return DayPlannerModel.fromJson(json.decode(http.Response(res,200).body));
    }
    print('noo');
    ...
    if (response.statusCode == 200) {
      print('suddjdjdjdjdj');
      return DayPlannerModel.fromJson(json.decode(response.body));
    } else {
      print('noo');
      throw Exception('unableToFetchDetails');
    }*/
